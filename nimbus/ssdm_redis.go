...
var _ = NimbusDescribe("ssdm redis service", func() {

	var appName, redisName, orgName string

	BeforeEach(func() {

		if Config.GetIncludeNimbusServiceSSDMRedis() != true {
			Skip("include_nimbus_service_ssdm_redis was not set to true")
		}
		...
		workflowhelpers.AsUser(TestSetup.AdminUserContext(), Config.DefaultTimeoutDuration(), func() {
			Expect(cf.Cf("enable-service-access", Config.GetNimbusServiceNameSSDMRedis(), "-p", "shared-vm", "-o", orgName).Wait(Config.DefaultTimeoutDuration())).To(Exit(0))
			Expect(cf.Cf("bind-security-group", "ssdm_redis_service", orgName).Wait(Config.DefaultTimeoutDuration())).To(Exit(0))
		})

		workflowhelpers.AsUser(TestSetup.RegularUserContext(), Config.DefaultTimeoutDuration(), func() {
			Expect(cf.Cf("create-service", Config.GetNimbusServiceNameSSDMRedis(), "shared-vm", redisName).Wait(Config.DefaultTimeoutDuration())).To(Exit(0))
