...
var _ = Describe("Copy app bits", func() {
	...
	BeforeEach(func() {
		...
		Expect(cf.Cf("push", golangAppName, "-b", config.RubyBuildpackName, "-m", "128M", "-p", assets.NewAssets().Golang, "--no-start", "-d", config.AppsDomain).Wait(DEFAULT_TIMEOUT)).To(Exit(0))
