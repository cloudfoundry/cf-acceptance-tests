...
var _ = Describe("Process Types", func() {
	...
	Describe("Staging", func() {
		...
		Describe("without a procfile", func() {
			BeforeEach(func() {
				Expect(cf.Cf("push", appName, "-b", config.NodejsBuildpackName, "-m", "128M", "-p", assets.NewAssets().Node, "-c", "node server.js --cool-arg", "-d", config.AppsDomain).Wait(CF_PUSH_TIMEOUT)).To(Exit(0))
			})
